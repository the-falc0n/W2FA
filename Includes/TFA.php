...
class TFA
{
    ...
    {
        if( $tfa_key ) {
            $uep     = wfta_get_user_password( $user_id );
            $tfa_key = Crypto::encrypt( $tfa_key, Key::unlock( $uep ) );
        }
        return \update_user_meta( $user_id, '_ski_wtfa_key', $tfa_key );
    }
    public static function get_user_wtfa_key( $user_id )
    {
        $tfa_key = \get_user_meta( $user_id, '_ski_wtfa_key', true );
        if( $tfa_key ) {
            $uep     = wfta_get_user_password( $user_id );
            $tfa_key = Crypto::decrypt( $tfa_key, Key::unlock( $uep ) );
        }
        return $tfa_key;
    }
}
